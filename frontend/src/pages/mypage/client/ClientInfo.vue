...
const placeholderText = computed(() => {
  return selectedEmailDomain.value === "self" ? "이메일 입력" : `이메일 입력 (예: 아이디@${selectedEmailDomain.value})`;
});
...
const openPostcodeModal = async () => {
  try {
    ...
    console.error("우편번호 스크립트 로드 실패:", error);
  }
};
...
const loadPostcodeScript = () => {
  if (document.querySelector('script[src="//t1.daumcdn.net/mapjsapi/bundle/postcode/prod/postcode.v2.js"]')) {
    return Promise.resolve(); // 이미 로드된 경우
  }
...
const cancelChanges = () => {
  const isConfirmed = confirm("정말로 수정을 취소하시겠습니까?");
  if (isConfirmed) {
    alert("수정이 취소되었습니다."); // 확인 시 동작
    // 실제 취소 로직을 여기에 추가 가능
    // 예: API 호출 후 목록 갱신
  }
};
...
const updateUserInfo = () => {
  const isConfirmed = confirm("정말로 회원 정보를 수정하시겠습니까?");
  if (isConfirmed) {
    alert("회원 정보가 수정되었습니다"); // 확인 시 동작
    // 실제 취소 로직을 여기에 추가 가능
    // 예: API 호출 후 목록 갱신
  }
};
